...
const defaultState = {
  ...
  audit: [],
  // per-login-username security state
  security: {
    // "ict.demo": { attempts: 0, lockedUntil: 0 }
  }
};
...
let pendingLogin = null;  // demo account object during login flow
let idleTimer = null;
...
// SAFETY GUARD: If no user is logged in yet, they have no rights.
function isRoleAtLeast(role){
  if (!current) return false;  // <<< important fix
...
// ====================================================================
// App wiring (nav, buttons, idle timeout)
// ====================================================================
function wireApp(){
  if(nav){
...
function startIdleTimer(){
  ...
  idleTimer = setTimeout(()=>{
    if(current){
      audit(`AUTO LOGOUT: idle timeout (5 min) user=${current.username}`);
      current = null;
      localStorage.removeItem(SESSION_KEY);
      showLogin();
...
function renderDashboard(){
  ...
  pPwSummary.textContent =
    `min ${state.policies.pwMinLen}` +
...
function renderPoliciesUI(){
  ...
  [polMfa, polRequireDevice, polCopilot, polQuota, polPwMinLen, polPwRequireNumber, polPwRequireUpper]
...
function renderUsers(){
  ...
  list.forEach(u=>{
    ...
    const isGA        = (current && current.role === ROLES.GLOBAL_ADMIN);
...
// SAFETY GUARD: renderAudit must not assume `current` exists.
function renderAudit(){
  auditList.innerHTML = "";

  if (!current) {
    auditList.innerHTML = `<li><span class="muted tiny">Sign in to view audit logs.</span></li>`;
    return;
  }

  if (!isRoleAtLeast(ROLES.HELPDESK)) {
    auditList.innerHTML =
      `<li><b>Access denied</b><br><span class="muted tiny">Audit logs require Helpdesk or higher.</span></li>`;
    return;
  }
...
// ====================================================================
// Data actions (Users / Policies / Audit / Export)
// ====================================================================
function addUser(){
  if(!isRoleAtLeast(ROLES.ICT_ADMIN)) return alert("Access denied.");
