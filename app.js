...
// Default state (directory + policies + audit)
const defaultState = {
  ...
  audit: [],
  security: {
    // username: { attempts: 0, lockedUntil: 0 }
  }
};
...
let pendingLogin = null;       // demo account waiting for MFA/Stay step
let idleTimer = null;
...
function loadState(){
  ...
  try { return JSON.parse(raw); } catch { return clone(defaultState); }
}

function isRoleAtLeast(role){
  return ROLE_ORDER.indexOf(current.role) >= ROLE_ORDER.indexOf(role);
}
function deviceIsCompliant(){
  return localStorage.getItem(DEVICE_FLAG) === "1";
}
function setDeviceCompliant(flag){
  localStorage.setItem(DEVICE_FLAG, flag ? "1" : "0");
}

// ===== Login wiring =====
function restoreDeviceCheckbox(){
  deviceCompliantFlag.checked = deviceIsCompliant();
}
...
function wireLogin(){
  ...
  btnNextToPassword.onclick = ()=>{
    ...
    if(!acct) return showError("Demo account not found (try ict.demo, global.demo)");
...
function afterStay(remember){
  ...
  if(state.policies.requireCompliantDevice && !deviceIsCompliant()){
    ...
    return showError("Device is not compliant (policy requires compliant device). Toggle it on the first login step.");
  }
...
function finalizeLogin(remember=false){
  ...
  if(remember){
    ...
    audit("SESSION: Remembered (stay signed in)");
  }else{
...
function ensureSec(u){
  if(!state.security[u]) state.security[u] = { attempts: 0, lockedUntil: 0 };
  return state.security[u];
}
function registerFail(u){
  const s = ensureSec(u);
  s.attempts += 1;
...
function roleCapabilities(role){
  ...
  if(role===ROLES.TEACHER)   return ["View users (read-only)"];
  if(role===ROLES.HELPDESK)  return ["Reset passwords (simulated)","Disable/enable users","View audit logs"];
  if(role===ROLES.ICT_ADMIN) return ["Manage users","Edit policies","Export/Import","View audit logs"];
  if(role===ROLES.GLOBAL_ADMIN) return ["Full tenant control","Change user roles","Edit all policies","View audit logs"];
  return ["Unknown role"];
}

function renderDashboard(){
  if(statUsers)  statUsers.textContent = String(state.users.length);
  if(statDevice) statDevice.textContent = deviceIsCompliant() ? "Yes" : "No";
  if(statMfa)    statMfa.textContent = state.policies.mfaRequired ? "Yes" : "No";

  pMfa.textContent     = state.policies.mfaRequired ? "Yes" : "No";
  pDevice.textContent  = state.policies.requireCompliantDevice ? "Yes" : "No";
  pQuota.textContent   = `${state.policies.onedriveQuotaGB} GB`;
  pCopilot.textContent = state.policies.copilotEnabled ? "Yes" : "No";
  pPwSummary.textContent = `min ${state.policies.pwMinLen}`
    + (state.policies.pwRequireNumber ? ", number" : "")
    + (state.policies.pwRequireUpper ? ", uppercase" : "");

  roleCaps.innerHTML = roleCapabilities(current.role).map(x=>`<li>${escapeHtml(x)}</li>`).join("");
}

function renderPoliciesUI(){
  polMfa.checked = !!state.policies.mfaRequired;
  polRequireDevice.checked = !!state.policies.requireCompliantDevice;
  polCopilot.checked = !!state.policies.copilotEnabled;
  polQuota.value = state.policies.onedriveQuotaGB ?? 10;
  polPwMinLen.value = state.policies.pwMinLen ?? 8;
  polPwRequireNumber.checked = !!state.policies.pwRequireNumber;
  polPwRequireUpper.checked  = !!state.policies.pwRequireUpper;

  const editable = isRoleAtLeast(ROLES.ICT_ADMIN);
  [polMfa, polRequireDevice, polCopilot, polQuota, polPwMinLen, polPwRequireNumber, polPwRequireUpper].forEach(el=>{
...
function renderUsers(){
  ...
  list.forEach(u=>{
    const canHelpdesk = isRoleAtLeast(ROLES.HELPDESK);
    const canAdmin    = isRoleAtLeast(ROLES.ICT_ADMIN);
    const isGA        = current.role === ROLES.GLOBAL_ADMIN;

    const roleCell = isGA
      ? `<select class="small" data-act="setrole" data-id="${u.id}">
    ...
    const actions = [];
    if(canHelpdesk){
      actions.push(`<button class="small" data-act="toggle" data-id="${u.id}">${u.active?"Disable":"Enable"}</button>`);
      actions.push(`<button class="small" data-act="resetpw" data-id="${u.id}">Reset PW</button>`);
    }
    if(canAdmin){
      actions.push(`<button class="small danger" data-act="del" data-id="${u.id}">Delete</button>`);
    }
    ...
    tr.innerHTML = `
      ...
      <td>${roleCell}</td>
      <td>${u.active ? `<span class="badge" style="color:#22c55e">Active</span>` : `<span class="badge" style="color:#f59e0b">Disabled</span>`}</td>
      <td>${actions.length ? actions.join(" ") : `<span class="muted">No actions</span>`}</td>
    `;
    ...
    tr.querySelectorAll("[data-act]").forEach(el=>{
      ...
      if(act === "toggle")  el.onclick = ()=> toggleUser(id);
      if(act === "resetpw") el.onclick = ()=> resetPassword(id);
      if(act === "del")     el.onclick = ()=> deleteUser(id);
      if(act === "setrole") el.onchange = (e)=> setUserRole(id, e.target.value);
    });
...
function addUser(){
  ...
  if(state.users.some(u=>u.email.toLowerCase() === email.toLowerCase())) return alert("Email already exists.");

  state.users.push({ id: uid(), name, email, role, active: true });
  saveState();
  audit(`USER CREATE: ${email} (role=${role})`);
...
function toggleUser(id){
  if(!isRoleAtLeast(ROLES.HELPDESK)) return alert("Access denied.");
  const u = state.users.find(x=>x.id===id); if(!u) return;
  u.active = !u.active;
  saveState();
  audit(`USER ${u.active?"ENABLE":"DISABLE"}: ${u.email}`);
  renderUsers();
}
...
function resetPassword(id){
  if(!isRoleAtLeast(ROLES.HELPDESK)) return alert("Access denied.");
  const u = state.users.find(x=>x.id===id); if(!u) return;
  const npw = prompt(`Enter new password for ${u.email}\n(Policy enforced)`);
  if(npw === null) return;

  const result = checkPasswordAgainstPolicy(npw);
  if(!result.ok) return alert(`Password does not meet policy:\n- ${result.reason}`);

  audit(`RESET PW (sim): ${u.email} • compliant with policy`);
  alert("Password reset simulated (policy OK).");
}
...
function setUserRole(id, newRole){
  if(current.role !== ROLES.GLOBAL_ADMIN) return alert("Only Global Admin can change roles.");
  const u = state.users.find(x=>x.id===id); if(!u) return;
  const oldRole = u.role;
  u.role = newRole;
  saveState();
  audit(`ROLE CHANGE: ${u.email} ${oldRole} → ${newRole}`);
  if(u.email === current.username){
    alert("You changed your own role. RBAC will apply immediately.");
  }
  renderUsers(); renderDashboard();
}

// ===== Password policy =====
function checkPasswordAgainstPolicy(pw){
  const p = state.policies;
  if(pw.length < p.pwMinLen) return { ok:false, reason:`Minimum length ${p.pwMinLen}` };
  if(p.pwRequireNumber && !/[0-9]/.test(pw)) return { ok:false, reason:"Must contain a number" };
  if(p.pwRequireUpper  && !/[A-Z]/.test(pw)) return { ok:false, reason:"Must contain an uppercase letter" };
  return { ok:true };
}
...
function savePolicies(){
  ...
  state.policies.requireCompliantDevice = !!polRequireDevice.checked;
  state.policies.copilotEnabled = !!polCopilot.checked;
  state.policies.onedriveQuotaGB = Math.max(1, parseInt(polQuota.value || "10", 10));
  state.policies.pwMinLen = Math.max(4, parseInt(polPwMinLen.value || "8", 10));
  state.policies.pwRequireNumber = !!polPwRequireNumber.checked;
  state.policies.pwRequireUpper  = !!polPwRequireUpper.checked;

  saveState();
  audit(`POLICY UPDATE: mfa=${state.policies.mfaRequired}, device=${state.policies.requireCompliantDevice}, quota=${state.policies.onedriveQuotaGB}, pw(min=${state.policies.pwMinLen}, num=${state.policies.pwRequireNumber}, upper=${state.policies.pwRequireUpper})`);
  renderDashboard();
...
function audit(message){
  state.audit.unshift({ time: now(), msg: message });
...
function renderAudit(){
  auditList.innerHTML = "";
  // Only Helpdesk+ can see
  if(!isRoleAtLeast(ROLES.HELPDESK)){
...
function exportJson(){
  ...
  a.download = "admin-sim-state.json";
  a.click();
  audit("EXPORT: Downloaded state");
}
...
async function importJson(e){
  ...
  try{
    ...
    if(!obj.users || !obj.policies || !obj.audit) throw new Error("Invalid format");
    state = obj;
    saveState();
    audit("IMPORT: Loaded state");
    renderAll();
